...
# Transforms text to vectors of integer numbers representing in text tokens and back. Handles word and character
# level tokenization.
class Vectorizer:
...
def generate(model, vectorizer, seed, length=100, diversity=0.5):
    ...
    model.reset_states()
    preds = None
    for char_index in np.nditer(seed_vector, ['zerosize_ok']):
...
if __name__ == "__main__":
    ...
    input_data_file = "input.txt"  # Source text file
    output_data_file = "output.txt"  # Source text file
    ...
    parser.add_argument("-to", "--text_output", required=False, help="Output text file")
    ...
    parser.add_argument("-uw", "--use_words", type=bool, default=True,
                        help="Use words instead of characters (slower speed, bigger vocabulary)")
    ...
    parser.add_argument("-s", "--save", type=bool, default=True,
                        help="Should or not save image result to results folder")

    args = parser.parse_args()
    if args.text is not None:
        input_data_file = args.text
    if args.text is not None:
        output_data_file = args.text_output
    ...
    x, y, x_val, y_val, vectorizer = load_data(input_data_file, use_words, pristine_input, pristine_output, batch_size,
                                               seq_length)

    model_file = input_data_file.lower().replace('.txt', '.h5')
